  ...
  async function createUser() {
    try {
      await api.post('/api/admin/users', {
        ...
        role: newRole, branchId: newBranchId || null
      })
  ...
  async function createBranch() {
    try {
      await api.post('/api/admin/branches', {
        name: branchName, address: branchAddress, city: branchCity
      })
                ...
                <div key={b.id} className="bg-white rounded-xl p-6 border border-gray-100">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      {(b as any).isMain && (
                        <span className="text-xs bg-brass/10 text-brass px-2 py-0.5 rounded font-medium border border-brass/20">Main</span>
                      )}
                    </div>
