    ...
    <div className="min-h-screen bg-[#F0F2F5]">
      <div className="fixed left-0 top-0 h-full w-56 bg-charcoal text-white flex flex-col p-6">
        ...
        <nav className="flex flex-col gap-2 flex-1">
          ...
          <div className="mt-4">
            <p className="text-xs text-gray-500 uppercase tracking-wider mb-2 px-2">Branches</p>
            {data?.branches.map(b => (
              <button
                key={b.branchId}
      ...
      <TopBar title="HQ Manager Dashboard" />
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-4 mb-8">
                  <div className="bg-white rounded-xl p-6 border border-gray-100">
                    ...
                    <p className="text-3xl font-bold text-charcoal">£{Number(data.grandTotalRevenue).toLocaleString()}</p>
                  </div>
                  <div className="bg-white rounded-xl p-6 border border-gray-100">
                    <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">Total Orders</p>
                    <p className="text-3xl font-bold text-charcoal">{data.branches.reduce((s, b) => s + b.totalOrders, 0)}</p>
                  </div>
                  <div className="bg-white rounded-xl p-6 border border-gray-100">
                    <p className="text-xs text-gray-400 uppercase tracking-wider mb-1">Active Branches</p>
                    <p className="text-3xl font-bold text-charcoal">{data.branches.length}</p>
                  </div>
                </div>
                <div className="bg-white rounded-xl p-6 border border-gray-100 mb-8">
                  <h3 className="font-semibold text-charcoal mb-6">Revenue by Branch</h3>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
                      <tr>
                        ...
                        <th className="text-left px-6 py-3">Today</th>
                    ...
                    <tbody>
                      {data.branches.map(b => (
                        <tr key={b.branchId} className="border-t border-gray-50 hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 font-semibold text-brass">£{Number(b.totalRevenue).toLocaleString()}</td>
                          <td className="px-6 py-4">
                            <button onClick={() => setSelectedBranch(b.branchId)} className="text-xs bg-charcoal text-white px-3 py-1 rounded-lg hover:opacity-80">View</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
            ...
            {activeTab === 'branch' && (
              <div>
                <div className="flex items-center gap-4 mb-8">
                  <button onClick={() => { setSelectedBranch('all'); setActiveTab('overview') }} className="text-sm text-gray-400 hover:text-charcoal">← Back</button>
                  <h2 className="text-2xl font-bold text-charcoal">{data?.branches.find(b => b.branchId === selectedBranch)?.branchName}</h2>
                </div>
                      ...
                      <tbody>
                        {branchOrders.slice(0, 20).map((o: any) => (
                          <tr key={o.id} className="border-t border-gray-50">
                            ...
                            <td className="px-6 py-3 font-medium text-brass">£{Number(o.totalAmount).toFixed(2)}</td>
                            <td className="px-6 py-3">
                              <span className={'text-xs font-medium px-2 py-1 rounded-full ' + statusColors[o.status]}>{o.status}</span>
                            </td>
                            <td className="px-6 py-3 text-gray-400">{new Date(o.createdAt).toLocaleDateString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
